...
WITH ATT AS (
    SELECT
        UPPER(TRIM(REPORT_ID)|| TRIM(ENTRY_KEY)|| TRIM(ENTRY_KEY_VER)) AS EXPENSE_ID
        ,
...
EXP_TIER AS (
    ...
    exp.EXPENSE_ID
        ,
    CASE
            WHEN UPPER(TRIM(EXPENSE_TYPE)) = 'CAR RENTAL' THEN exp.APPROVED_AMOUNT_RPT / MAX(1, EXP.CAR_RENTAL_DAYS)
            ELSE APPROVED_AMOUNT_RPT / COALESCE(ATT.NUM_ATTENDEES,
        1)
    ...
    TRIM(UPPER(EXP.ARRIVAL_AIRPORT)) AS ARRIVAL_AIRPORT
        ,
    COALESCE(VENDOR_CITY_LOCATION,
    EXP.EXPENSE_ID) AS VENDOR_CITY_LOCATION
        ,
    VENDOR_COUNTRY
        ,
    COALESCE(COALESCE(TRIM(UPPER(MERCHANT_COUNTRY_CODE)) ,
    TRIM(UPPER(EXP_CLST.COUNTRY_CODE))),
    TRIM(UPPER(EMP.COUNTRY_CODE))) AS MERCHANT_COUNTRY_CODE
        ,
    COALESCE(COALESCE(TRIM(UPPER(MERCH_CLST.COUNTRY_NAME)) ,
    ...
    TRIM(UPPER(EXP_CLST.COUNTRY_CODE)) AS EXP_CLST_COUNTRY_CODE
        ,
    COALESCE(EXP_CLST.EXPENSE_TIER_REGION,
    EMP_CLST.EXPENSE_TIER_REGION) AS EXP_CLST_REGION
        ,
	...
	EC.REPORT_ID || EC.ENTRY_KEY || EC.ENTRY_KEY_VER IS NOT NULL
        ...
        END AS IS_HCAM
    FROM
        EI_INFO.EXPENSES EXP
    JOIN EI_PROFILE.EMPLOYEES EMP ON
        EMP.EMPLOYEE_ID = EXP.EMPLOYEE_ID
    LEFT JOIN EI_INFO.CREDIT_TRANSACTIONS TRANS ON
    ...
    JOIN LOOKUP.GEOGRAPHY_INFO EXP_CLST ON
        TRIM(UPPER(EXP_CLST.COUNTRY_CODE)) = TRIM(UPPER(EXP.VENDOR_COUNTRY))
    LEFT JOIN LOOKUP.GEOGRAPHY_INFO MERCH_CLST ON
        COALESCE(COALESCE(TRIM(UPPER(MERCHANT_COUNTRY_CODE)) ,
    TRIM(UPPER(EXP_CLST.COUNTRY_CODE))),
    TRIM(UPPER(EMP.COUNTRY_CODE))) = UPPER(TRIM(MERCH_CLST.COUNTRY_CODE))
    LEFT JOIN LOOKUP.GEOGRAPHY_INFO EMP_CLST ON
        EMP_CLST.COUNTRY_CODE = UPPER(TRIM(EMP.COUNTRY_CODE))
    LEFT JOIN LOOKUP.MERCHANT_CODE_NAMES DESCRP ON
        TRANS.MERCHANT_CODE = DESCRP.MERCHANT_CODE
    LEFT JOIN ATT ON
        ATT.EXPENSE_ID = EXP.EXPENSE_ID
    LEFT JOIN TRAVEL_RAW_LANDING.MCOFFICER M ON
        EMP.EMPLOYEE_ID = M.EMPCNUM
    LEFT JOIN DISTINCT_EXPENSE_COMMENTS EC ON
...
SELECT
    ...
    ,
    CASE
        WHEN BASELINE_PARENT_EXPENSE_TYPE = 'FLIGHT' THEN
        CASE
            WHEN DEPARTURE_AIRPORT IS NULL
            OR ARRIVAL_AIRPORT IS NULL THEN EXPENSE_ID
